...
// CREDIT: https://gist.github.com/niik/1017834
...
public sealed class CryptoRandom : Random, IDisposable
{
    ...
    readonly object _syncBlock = new();
    ...
    void InitBuffer()
    {
        ...
        {
            if (_buffer is null || _buffer.Length != 512)
                _buffer = new byte[512];
        }
        else
        {
            if (_buffer is null || _buffer.Length != 4)
                _buffer = new byte[4];
        }
    ...
    public override void NextBytes(byte[] buffer)
    {
        ...
        lock (_syncBlock)
        {
            if (IsRandomPoolEnabled && _buffer is null)
                InitBuffer();
    ...
    uint GetRandomUInt32()
    {
        lock (_syncBlock)
        {
            ...
            // We know _buffer can't be null here, but the compiler doesn't.
            // So we use the ! bang to tell it.
