...
public class SharedChannelInviteDeclinedTests
{
    [Fact]
    public void CanBeDeserialized()
    {
        ...
        var result = Assert.IsAssignableFrom<IEventEnvelope<SharedChannelInviteDeclined>>(envelope);
        var eventBody = result.Event;
        Assert.Equal("I05M5GCNVNW", eventBody.Invite.Id);
        Assert.Equal("C05M7V86LNQ", eventBody.Channel.Id);
        Assert.NotNull(eventBody.DecliningUser);
        Assert.Equal("UFW4S427J", eventBody.DecliningUser.Id);
        Assert.Equal("TFWSDE3AN", eventBody.DecliningUser.TeamId);
        Assert.Equal("TFWSDE3AN", eventBody.DecliningTeamId);
    }
}
