...
public class SharedChannelInviteApprovedTests
{
    [Fact]
    public void CanBeDeserialized()
    {
        ...
        var result = Assert.IsAssignableFrom<IEventEnvelope<SharedChannelInviteApproved>>(envelope);
        var eventBody = result.Event;
        Assert.Equal("I05M5GCNVNW", eventBody.Invite.Id);
        Assert.Equal("C05M7V86LNQ", eventBody.Channel.Id);
        Assert.NotNull(eventBody.ApprovingUser);
        Assert.Equal("UFW4S427J", eventBody.ApprovingUser.Id);
        Assert.Equal("TFWSDE3AN", eventBody.ApprovingUser.TeamId);
        Assert.Equal("TFWSDE3AN", eventBody.ApprovingTeamId);
    }
}
